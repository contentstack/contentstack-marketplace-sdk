...
import ContentstackRegion from './core/contentstackRegion.js'
...
export default function contentstackClient ({ http }) {
  ...
  let region = http.defaults.region
  let baseUrlPath = `https://api.contentstack.io:443/v3/user-session`

  if (region === ContentstackRegion.EU || region === ContentstackRegion.US || region === ContentstackRegion.AZURE_EU || region === ContentstackRegion.AZURE_NA) {
    region = http.defaults.region + '-'
    baseUrlPath = `https://${region}api.contentstack.io:443/v3/user-session`
  }
