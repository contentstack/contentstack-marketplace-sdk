...
const SDK = `contentstack-marketplace-javascript/${packages.version}`
...
describe('Contentstack HTTP Client', () => {
  ...
  it('Contentstack Http Client X-User-Agent Header', done => {
    ...
    createClientRewireApi.__Rewire__('contentstackClient', sinon.stub().returns({}))
    client()
    expect(createHttpClientStub.args[0][0].defaultHostName).to.be.equal('developerhub-api.contentstack.com', 'Default host name not match')
  ...
  it('Contentstack Http Client User-Agent Header', done => {
    ...
    expect(headerParts[0]).to.be.match(/^sdk contentstack-marketplace-javascript\/.+/, 'User agent sdk does not match')
  ...
  it('Contentstack Http Client User-Agent Header Custom', done => {
    ...
    expect(headerParts[3]).to.be.match(/^sdk contentstack-marketplace-javascript\/.+/, 'User agent sdk does not match')
    createClientRewireApi.__ResetDependency__('httpClient')
    createClientRewireApi.__ResetDependency__('contentstackClient')
    done()
  })
